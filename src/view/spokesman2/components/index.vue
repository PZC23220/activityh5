            ...
            shareSns(val) {
                ...
                if(self.isFans) {
                    if(location.hostname == 'activity.groupy.vip') {
                        var shareURL = `https://share.groupy.vip/html/activity_spokesman2/index.html?activityId=${getParams('activityId')}&idolId=${self.me.idol_id}`;
                    }else {
                        var shareURL = `https://share.groupy.cn/html/activity_spokesman2/index.html?activityId=${getParams('activityId')}&idolId=${self.me.idol_id}`;
                    }
                    var title = `【イメージガール大募集】`;
                    var description = `上位1名がGroupy日本各アプリストアの画像に登場！動画がいっぱい投稿したので、応援してね。`;
                    var shareImg = `https://photoh5-jp.oss-ap-northeast-1.aliyuncs.com/acticity_banner/activity-spokesman2.jpg`;
                }else {
                    if(location.hostname == 'activity.groupy.vip') {
                        var shareURL = `https://share.groupy.vip/html/activity_spokesman2/index.html?activityId=${getParams('activityId')}&isFans=1`;
                    }else {
                        var shareURL = `https://share.groupy.cn/html/activity_spokesman2/index.html?activityId=${getParams('activityId')}&isFans=1`;
                    }
                    var title = `【イメージガール大募集】`;
