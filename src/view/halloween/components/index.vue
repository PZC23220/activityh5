          ...
          shareSns(val) {
            ...
            if(self.isFans) {
                if(location.hostname == 'activity.groupy.vip') {
                    var shareURL = `https://share.groupy.vip/html/activity_halloween/index.html?activityId=${getParams('activityId')}&idolId=${self.me.idol_id}`;
                }else {
                    var shareURL = `https://share.groupy.cn/html/activity_halloween/index.html?activityId=${getParams('activityId')}&idolId=${self.me.idol_id}`;
                }
                var title = `Groupyハロウィン動画コンテスト`;
                var description = `起動画面・イベントバナーに登場できるイベント参加中、動画をいっぱい投稿したので、応援してね`;
                var shareImg = `https://photoh5-jp.oss-ap-northeast-1.aliyuncs.com/acticity_banner/activity-halloween.jpg`;
            }else {
                if(location.hostname == 'activity.groupy.vip') {
                    var shareURL = `https://share.groupy.vip/html/activity_halloween/index.html?activityId=${getParams('activityId')}&isFans=1`;
                }else {
                    var shareURL = `https://share.groupy.cn/html/activity_halloween/index.html?activityId=${getParams('activityId')}&isFans=1`;
                }
                var title = `Groupyハロウィン動画コンテスト`;
