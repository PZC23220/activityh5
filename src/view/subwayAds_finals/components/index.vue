...
<script>
    ...
    export default {
        ...
        methods: {
            shareSns(val) {
                ...
                if(location.hostname == 'activity.groupy.vip') {
                    var shareURL = `https://share.groupy.vip/html/activity_subwayAds_finals/index.html?activityId=${getParams('activityId')}&isFans=1`;
                }else {
                    var shareURL = `https://share.groupy.cn/html/activity_subwayAds_finals/index.html?activityId=${getParams('activityId')}&isFans=1`;
                }
                var title = `【Groupy駅看板モデル選定企画-決勝`;
                var description = `上位3名は中国広州大型駅看板に掲載！動画がいっぱい投稿されているので、応援しよう。`;
                var shareImg = `https://photoh5-jp.oss-ap-northeast-1.aliyuncs.com/acticity_banner/activity-subwayAds_finals.jpg`;
        ...
        created() {
            ...
            window.setupWebViewJavascriptBridge(function(bridge) {
                bridge.callHandler('setTitle', {'title':'Groupy駅看板モデル選定企画-決勝'})
            })
        }
    }
</script>
