            ...
            shareSns(val) {
                ...
                if(self.isFans) {
                    if(location.hostname == 'activity.groupy.vip') {
                        var shareURL = `https://share.groupy.vip/html/activity_spokesman3/index.html?activityId=${getParams('activityId')}&idolId=${self.me.idol_id}`;
                    }else {
                        var shareURL = `https://share.groupy.cn/html/activity_spokesman3/index.html?activityId=${getParams('activityId')}&idolId=${self.me.idol_id}`;
                    }
                    var title = `【#Groupyイメージガール3 】`;
                    var description = `桜をテーマにしたGroupy日本語版イメージガールになろう限定企画に参加した！応援してね。#Groupy`;
                    var shareImg = `https://photoh5-jp.oss-ap-northeast-1.aliyuncs.com/acticity_banner/activity-spokesman3.png`;
                }else {
                    if(location.hostname == 'activity.groupy.vip') {
                        var shareURL = `https://share.groupy.vip/html/activity_spokesman3/index.html?activityId=${getParams('activityId')}&isFans=1`;
                    }else {
                        var shareURL = `https://share.groupy.cn/html/activity_spokesman3/index.html?activityId=${getParams('activityId')}&isFans=1`;
                    }
                    var title = `【#Groupyイメージガール3 】`;
